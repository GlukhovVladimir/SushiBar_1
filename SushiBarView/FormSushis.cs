...
namespace SushiBarView
{
    public partial class FormSushis : Form
    {
        ...
        {
            if (SushiGridView.SelectedRows.Count == 1)
            {
                var form = Container.Resolve<FormSushi>();
                form.Id = Convert.ToInt32(SushiGridView.SelectedRows[0].Cells[0].Value);
                if (form.ShowDialog() == DialogResult.OK)
                {
                    LoadData();
                }
        ...
        private void Delete_button_Click(object sender, EventArgs e)
        {
            if (SushiGridView.SelectedRows.Count == 1)
            {
                ...
                {
                    int id = Convert.ToInt32(SushiGridView.SelectedRows[0].Cells[0].Value);
                    try
                    {
