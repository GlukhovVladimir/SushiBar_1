...
namespace SushiBarView
{
    public partial class FormSushi : Form
    {
		...
		public FormSushi(ISushiLogic logic)
		{
			InitializeComponent();
			this.logic = logic;
		}
		...
		private void FormSushi_Load(object sender, EventArgs e)
		{
			if (id.HasValue)
			{
				try
				{
					var view = logic.Read(new SushiBindingModel { Id = Convert.ToInt32(id) })?[0];
					if (view != null)
					{
						Name_textBox.Text = view.SushiName;
					}
				}
				catch (Exception ex)
				{
